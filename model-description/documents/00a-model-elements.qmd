...
### Scale
Model grid cells represent 400&times;400 meter (16 hectare) areas. Model time steps represent 5 years of elapsed time. Note that although model time progresses in 5 year intervals, estimates of farm income, costs, profits, debt payments, etc. are annualised.
...
A list `interventions` of potential management interventions or practices, such as e.g., `"Build_Wetland"`, `"Clean_Races"`, `"Farm_Plan"`, `"Join_ETS"`, or `"Riparian_Planting"`. These have associated impacts on costs, yields and emissions as set out in the [`intervention-impacts.csv` file](05-farmer-decisions-interventions.qmd#the-effect-of-interventions).
...
[^2]: Termed 'turtles' in NetLogo due to NetLogo's origins in the Logo programming language which introduced 'turtle graphics'. See Resnick M. 1994. _Turtles, Termites, and Traffic Jams: Explorations in Massively Parallel Microworlds_. Cambridge MA: MIT Press. Everything being an agent means that farms, holdings, and so on are also 'agents' but not really in the sense implied by work programs such as the _Agents of Change_ RA in _Moving the Middle_.
...
+ `my-local-link-neighbors`, `my-catchment-link-neighbors`, and `my-disposition-link-neighbors`: other farmers connected to this one, that might influence their decisions.
...
### Farms
Farms are mostly the level at which income and costs are aggregate and debt is paid off to give the final profit earned by the owning farmer. Farm attributes include:
...
+ `loan-principal`: initial principal amount of any debt on the farm.
...
+ `the-land`: a list of patches that compose the holding.
