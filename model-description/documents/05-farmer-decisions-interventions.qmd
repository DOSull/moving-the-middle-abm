...
For each intervention $i_i$ there are associated impacts, in terms of added costs $\Delta x_{iFL}$, and changes in yields $\Delta q_{iFL}$ and emissions $\Delta e_{iFL}$, where the $F$ and $L$ subscripts indicate the dependence of these impacts on farm type $F$ and land use capability $L$. The impacts of various interventions are stored in the `intervention-details.csv` files, in corresponding rows. For example, the `costs` and `yields` look like this:

```{r}
interventions <- read.csv(str_glue("{base_dir}/intervention-details.csv"), header = TRUE)
interventions |> 
  dplyr::filter(statistic == "costs" | statistic == "yields") |>
  kable() |> 
...
| Date | Changes
|:-|:------
| 2025-10-21 | Fixed for change to where intervention impacts are stored.
| 2025-10-18 | Fixed for some changes to input file names.
