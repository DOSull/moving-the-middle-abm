---
title: "Holding income and costs"
subtitle: "Farm profit and loss is the key driver of change"
...
Each of these parameters may depend on the [farm type](01-model-elements.qmd#land-use) $F$ and the land use capability (LUC), $L$, such that the full calculation of profit for each hectare of a holding is
...
In the remainder of this description we assume only greenhouse gases are taxed and use the simplified form $te$ for the environmental tax burden in preference to the more complete sum shown in eqn.[-@eq-detailed].
...
### Or visually...
Below are shown the income, costs, and net revenue per hectare for each farm type across the eight land use capability classes. As seen below, these are crude estimates dating back to the ARLUNZ model^[See: Morgan FJ and AJ Daigneault. 2015. [Estimating Impacts of Climate Change Policy on Land Use: An Agent-Based Modelling Approach](https://dx.doi.org/10.1371/journal.pone.0127317). _PLOS ONE_ **10**(5): e0127317.] and will be updated to reflect more recent data. Even so it is expected that the relative profitability of the different broad land use classes will remain similar.
...
Prices are stored as a prices vector, $\mathbf{p}=\left[p_i\right]$, and the carbon tax as a scalar value, $t$, since it is fixed regardless of farm type.
...
A more involved matrix formulation is required for the impact of adoption of interventions on emissions and yields because these are combined multiplicatively. The model code is the best guide to the details in this case.
...
| 2026-05-05 | Equations numbered, some links to other pages fixed. Title changed. Added note on matrix calculations.
:::
